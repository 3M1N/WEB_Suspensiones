...
jQuery(document).ready(function($){
	...
	*/
	var doc = $(document),
		body = $("body"),
		img = $("#content img:not(.wb_dayd_latests_widget img)"),
		dayd_menu = $("body > header"),
	...
	/*
		DOM manips
	*/	
	dayd_menu.find(".nav > li:has(ul) > a").append("<span></span>");
	...
	/*
		CSS/UI adjustments
	*/	
	dayd_menu.find(".nav > li > ul").each(function(){
	...
	/*
		UI animations
	*/
	dayd_menu.find(".nav ul li a").hover(function(){
	...
	*/
	...
	/*
		Sticky menu
	*/
	if ( dayd_menu.hasClass("sticky_header") ) {
		
		$(window).scroll(function(){
			
			dayd_menu_height = dayd_menu.outerHeight(),
			scrolltop = $(this).scrollTop();
			
			if ( scrolltop > dayd_menu_height * 2 ) {
				
				if ( dayd_menu.hasClass("sticky") ) return;
				
				body.css("padding-top", dayd_menu_height );
				
				dayd_menu.addClass("sticky").css("top", -150).animate({ top: 0 }, { duration: 1200, queue: false, easing: "easeOutExpo" });
	...
	function dayd_hero_jcycle() {
		...
		dayd_hero_slider
			...
			.cycle({ 
			    ...
			    pause: dayd_slider.pause,
				slideExpr: ".slide",
			    containerResize: false,
				manualTrump: false,
			    next: "#hero .jcycle_arrow.next",
			    prev: "#hero .jcycle_arrow.prev",
