...
if (isset($_POST['enviar'])) {
  ...
  if (!$resp->is_valid) {
    ...
    die ("El reCAPTCHA no fue introducido correctamente, vuelva a la p&aacute;gina anterior e intentelo de nuevo. <a href='javascript:history.back(1)'>Volver Atrás</a>" .
	...
	$mail->Subject = "Solicitud de pedido";//"Subject del correo";
	...
	$body  = "<em>Solicitud de empleo enviada desde www.suspensiones.mx por:</em> <br />
					...
					<br />Tel&eacute;fono: ".utf8_decode($_POST['telefono'])."
					<br />N&uacute;mero(s) de parte: ".utf8_decode($_POST['asunto'])."
					<br />Archivo adjunto:
	...
	echo '<h2 style=" font-size: 15px; font-style: italic; font-weight: normal;">Mensaje entregado.</h2>
		...
		echo '<h2 style=" font-size: 15px; font-style: italic; font-weight: normal;">Mensaje sin entregar.</h2>
                                        <p>Algo realmente extraño ocurri&acute;, y no se pudo entregar tu mensaje.</p>';
	}
  }
...
<form action="<? $_SERVER['PHP_SELF'] ?>" method="post" name="contactForm" enctype="multipart/form-data">
	<script>
		...
		function VerifyLength(form,minimo){
			...
			alert("Tu nombre o telefono no esta completo.");
			}
		}
	</script>
	<div class="clear">
	...
	<div class="">
		<br/>
		<label for="-button">Tel&eacute;fono (Necesario):</label>
		<br/>
		<input name="telefono" id="telefono" class="" type="text" onChange="VerifyLength(this.form, 3)" />
	</div>
	<div class="clear"></div>
	<div class="">
		<br/>
		<label for="-button">N&uacute;mero(s) de parte:</label>
		<br/>
		<input name="asunto" id="asunto" class="" type="text">
	</div>
	<div class="">
		<br/>
		<label for="-button">Adjuntar lista o archivo de pedido:</label>
		<br/>
	...
	<div class="row field_textarea">
		<br/>
		<label for="-button">Indicaciones/Comentarios adicionales:</label>
		<br/>
